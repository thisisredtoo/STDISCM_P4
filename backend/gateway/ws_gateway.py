...
# =============================
# REWORKED ws_gateway.py
# =============================
# This version uses the shared broadcaster and removes the isolated clients set.


# =============================
# ws_gateway.py (REWORKED)
# =============================
...
async def http_publish(obj: dict = Body(...)):
    ...
    await broadcaster.publish_json(obj)
    return {"ok": True}
...
async def rpc_bridge(cmd: dict):
    try:
        ...
        await broadcaster.publish_json({"type": "control", "payload": cmd})
        return JSONResponse(
            {"ok": False, "error": "TrainingControl not available; broadcast only"},
            status_code=501,
        )

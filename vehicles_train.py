...
def send_batch_update(images, predictions, ground_truths, index=0):
    try:
        image_msgs = [dashboard_pb2.ImageData(image=(img*255).astype(np.uint8).tobytes()) for img in images]
        dashboard_stub.SendBatchUpdate(dashboard_pb2.BatchUpdate(
            index=index,
            images=image_msgs,
            predictions=predictions,
            groundTruths=ground_truths
...
def send_training_status(is_training, message):
    try:
        training_stub.TrainingStatusUpdate(training_pb2.TrainingStatus(
            isTraining=is_training,
            statusMessage=message
        ))
    except grpc.RpcError as e:
        print("gRPC TrainingStatus error:", e)


def send_training_metrics(iteration, loss_value):
    try:
        training_stub.TrainingMetricsUpdate(training_pb2.TrainingMetrics(
            currentIteration=float(iteration),
            currentLoss=float(loss_value)
        ))
    except grpc.RpcError as e:
...
for epoch in range(EPOCHS):
    ...
    for step in range(steps_per_epoch):
        ...
        # -------- gRPC UPDATES --------
        send_loss_update(loss_value, iteration)                 # dashboard.proto
        send_training_metrics(iteration, loss_value)            # training.proto
        ...
        iteration += 1
        time.sleep(0.15)  # small delay per batch (can adjust)
send_training_status(False, "Training completed successfully")
